<?php
include("dictionary_list_class.php");

class dictionary_list_manager
{
		//activate single reecord
		function activate_single($id)
		{
			global $db;
			$up_sql = "UPDATE tbl_dictionary SET is_active = 'Y' WHERE id='$id'";
	...
	//De-activate single reecord
		...
		{
			   global $db;
			   $up_sql = "UPDATE tbl_dictionary SET is_active = 'N' WHERE id='$id'";
			   $res_sql = $db->update_data($up_sql);
	           if(count($res_sql)>0)
				{
		...
		function delete_selected($adminObject)
		{
			...
			for($i=0;$i<count($delete_id);$i++)
			{
				...
				if($del != "")
				{
					$sql_delete		=	"DELETE FROM `tbl_dictionary` WHERE id='".$del."'";
		...
		function activate_selected($adminObject)
		{
			...
			{
				 $sql_update	=	"UPDATE `tbl_dictionary` SET is_active='Y' WHERE id='".$status_id[$i]."'";				
		...
		function deactivate_selected($adminObject)
		{
			...
			for($i=0;$i<count($status_id);$i++)
			{
			    $sql_update		=	"UPDATE `tbl_dictionary` SET is_active='N' WHERE id='".$status_id[$i]."'";
				$res_update		=	$db->update_data($sql_update);	
			} // for i
	...
	//search member
		...
		{
			 global $db;
		     $sql 		  =	"select * from tbl_dictionary ORDER BY `id` DESC";
		...
		{
			global $db;
		    $sql_delete		=	"DELETE FROM tbl_dictionary WHERE id='".$delete_id."'";
		    $res_delete		=	$db->delete_data($sql_delete);
