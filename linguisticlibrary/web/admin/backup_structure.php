...
$query="SHOW CREATE TABLE `tbl_dictionary`";
 ...
 //$cnt=count($result);
	...
	//	$outputdata .= "\n\n-- Dumping data for table : $tables[$t]\n\n";
		...
		while($row = mysql_fetch_assoc($result)){
			...
			$lines .= "INSERT INTO `tbl_dictionary` VALUES (";
			for($i=0;$i<$nor;$i++){
				//echo $datas[$i];
				if($datas[$i]===NULL){
					$lines .= "NULL";
...
//	}
	...
	//$len = fwrite($file,$datadump);
     ...
     header('Content-Disposition: attachment; filename="'."tbl"."_"."dictionary_".date('YmdHis').'.sql"');
